...
export function NewTaskView({
  ...
  useEffect(() => {
    ...
    loadSendShortcut();
    window.addEventListener("fastaf:app-settings-changed", loadSendShortcut);
    return () => window.removeEventListener("fastaf:app-settings-changed", loadSendShortcut);
  }, []);
  ...
  // Hook readiness state: softly warn the user when the version is too low / node is missing (the task can still launch, having fallen back to polling).
  ...
  function handleInitializeMd() {
    ...
    // Initializing the md file involves no code changes, so force local to avoid needless worktree overhead
    onSubmit({
      kind: "agent",
      prompt,
      agent,
  ...
  return (
    <div style={s.newTaskOuter}>
      ...
      {/* Launch mode + base branch (outside the compose card, its own row) */}
