...
export function RunningView({
  ...
  // Pure terminal task: no agent session / token metrics, so use a compact single-line header.
  const isShell = (task.kind ?? "shell") === "shell";
  const isActive =
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Keep only CJK chars/letters/digits/hyphens; replace others with _ to avoid illegal filename characters.
      const slug =
        titleSource
          .slice(0, 50)
          .replace(/[^\w\u4e00-\u9fa5-]+/g, "_")
      ...
      const defaultName = `fastaf-${slug}-${date}.md`;
  ...
  useEffect(() => {
    ...
    // Only run metrics polling while the project is in the foreground; pause when switching to another project,
    // and immediately refetch once here when the project is reactivated. Note this uses projectActive
    // rather than visible — the latter is also false when opening FileViewer / GitDiff within the same project,
    // and in that case we must not interrupt the duration updates of a running task.
  ...
  return (
    ...
    >
      {/* Header — hidden for plain-terminal tasks (full-bleed native terminal) */}
      {!isShell && (
      <div
        style={s.runHeader}
      ...
      {!isShell && !isActive && !isDetached && !isInterrupted && !sessionPath && (
        <div
          style={{
            padding: "10px 20px",
