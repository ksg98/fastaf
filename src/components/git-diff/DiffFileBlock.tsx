...
const SPLIT_CELL_GRID: CSSProperties = {
  ...
  fontSize: 12.5,
  lineHeight: "22px",
  // Clip overly long lines so whiteSpace:pre content doesn't overflow this column past the center divider into the other column
  overflow: "hidden",
};
...
// Lazy-rendering container for a single hunk: renders no content by default (only occupies a placeholder height),
// mounting the real DOM when it enters the viewport; once rendered it stays mounted, avoiding repeated unmount flicker while scrolling.
function LazyHunkBody({
  ...
  // To avoid scroll jitter from too large a gap between placeholder and actual height, use row count × 22px as the minimum height.
