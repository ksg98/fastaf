...
// Count of tasks awaiting confirmation (input_required) — used for the yellow count badge
...
// Project status indicator: shows a count badge when the badge is enabled and there are tasks awaiting confirmation, otherwise falls back to a small dot.
// borderColor is used to blend the stroke with the containing element's background (rail and drawer backgrounds differ).
...
function RailItem({
  ...
  // Each time waveNonce increments (a new task awaiting confirmation appears) it triggers a one-shot wave, unmounted after 3.6s.
  // Unmounting + remounting replays the gif from the first frame and restarts the CSS peek/retract animation.
  useEffect(() => {
...
export function ProjectRail({
  ...
  // The rail only shows pinned projects; the currently active project is always kept even if marked non-pinned, to avoid losing the current context.
  const railProjects = useMemo(
    () => projects.filter((p) => !p.hiddenFromRail || p.id === activeProjectId),
    [projects, activeProjectId],
  );

  // Wave trigger: record each project's previous awaiting-confirmation count; when the count increases (0→≥1 or n→n+1),
  // increment a nonce for that project, which RailItem uses to play a one-shot wave animation. The first frame only seeds initial values and does not wave for existing tasks.
