...
export function useProjectPanels(filesPanelDefaultOpen = false) {
  // The file panel's default open state is determined by a setting (only applies on initial mount; afterward toggled manually by the user / Cmd+B).
  const [rightPanel, setRightPanel] = useState<RightPanel>(
    filesPanelDefaultOpen ? "files" : null,
  );
