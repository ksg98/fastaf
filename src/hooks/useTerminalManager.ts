...
const DRAIN_FRAME_BUDGET = 128 * 1024; // process at most 128KB per frame, to avoid a single frame's write taking too long
...
export function useTerminalManager() {
  ...
  // Deliver directly to a single subscriber via tauri::ipc::Channel, bypassing the global emit/listen event bus.
  // pendingOutputs / RAF are still shared at the hook level, preserving the original batched-write cadence and per-frame byte budget.
