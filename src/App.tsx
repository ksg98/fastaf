...
function getInitialThemeMode(): ThemeMode {
  const stored = localStorage.getItem("fastaf:theme");
  return stored === "dark" ||
    stored === "light" ||
    stored === "system" ||
    stored === "eyecare" ||
...
function getInitialTerminalFontSize(): TerminalFontSize {
  const stored = localStorage.getItem("fastaf:terminalFontSize");
...
function getInitialTaskDisplayWindow(): TaskDisplayWindow {
  const stored = localStorage.getItem("fastaf:taskDisplayWindow");
...
function getInitialAttentionBadge(): boolean {
  // On by default: the project rail shows a count badge of tasks awaiting confirmation; when off, falls back to a small yellow dot
  return localStorage.getItem("fastaf:attentionBadge") !== "0";
}

function getInitialFilesPanelOpen(): boolean {
  // On by default: auto-expand the right-side file browser when entering a project
  return localStorage.getItem("fastaf:filesPanelDefaultOpen") !== "0";
}

function getInitialFontFamily(key: string, fallback: FontFamily): FontFamily {
  const stored = localStorage.getItem(key);
  if (!stored) return fallback;
  // Bare font names written by older versions (e.g. "Maple Mono NF CN") get tokenized into
  // multiple families under Canvas 2D and all miss; normalize once on read.
...
function App() {
  ...
  const [filesPanelDefaultOpen, setFilesPanelDefaultOpen] =
    useState<boolean>(getInitialFilesPanelOpen);
  const [uiFontFamily, setUiFontFamily] = useState<FontFamily>(() =>
    getInitialFontFamily("fastaf:uiFontFamily", DEFAULT_UI_FONT),
  );
  const [monoFontFamily, setMonoFontFamily] = useState<FontFamily>(() =>
    getInitialFontFamily("fastaf:monoFontFamily", DEFAULT_MONO_FONT),
  );
  ...
  useLayoutEffect(() => {
    ...
    localStorage.setItem("fastaf:theme", themeMode);
  ...
  useEffect(() => {
    // VS Code-style page zoom Cmd/Ctrl +/-/0. Handle deterministically in the capture phase, ahead
    // of xterm, replacing the zoomHotkeysEnabled polyfill (the "-" key detection is unstable on macOS).
    restoreZoom();
    function handleZoom(event: KeyboardEvent) {
      const intent = classifyZoomShortcut(event, APP_PLATFORM);
      if (!intent) return;
      event.preventDefault();
      if (intent === "in") zoomIn();
    ...
    return () => window.removeEventListener("keydown", handleZoom, true);
  }, []);

  useEffect(() => {
    localStorage.setItem("fastaf:terminalFontSize", String(terminalFontSize));
  }, [terminalFontSize]);

  useEffect(() => {
    localStorage.setItem("fastaf:taskDisplayWindow", String(taskDisplayWindow));
  }, [taskDisplayWindow]);

  useEffect(() => {
    localStorage.setItem("fastaf:attentionBadge", attentionBadge ? "1" : "0");
  }, [attentionBadge]);

  useEffect(() => {
    localStorage.setItem("fastaf:filesPanelDefaultOpen", filesPanelDefaultOpen ? "1" : "0");
  }, [filesPanelDefaultOpen]);

  useEffect(() => {
    const value = uiFontFamily.trim() || DEFAULT_UI_FONT;
    localStorage.setItem("fastaf:uiFontFamily", value);
    document.documentElement.style.setProperty("--font-ui", value);
  }, [uiFontFamily]);

  useEffect(() => {
    const value = monoFontFamily.trim() || DEFAULT_MONO_FONT;
    localStorage.setItem("fastaf:monoFontFamily", value);
  ...
  useEffect(() => {
    async function init() {
      // Load projects from ~/.fastaf/projects.json
  ...
  useEffect(() => {
    // Treat the backend list as authoritative and merge it into frontend state:
    // the backend-written version overrides shared items; entries unique to the frontend but not yet
    // persisted by the backend are kept.
    ...
    const handleSkillHubChanged = (e: Event) => {
      ...
      if (detail?.projects && Array.isArray(detail.projects)) {
        // Sync path: set_skill_hub_path already returned the full list, so merge directly to avoid a race
        invoke<SkillHubConfig>("get_skill_hub_config")
      ...
      // Scenarios like clear_skill_hub have no projects payload, so fall back to a full reload
      loadFromBackend();
    };
  ...
  useEffect(() => {
    const p1 = listen<{ task_id: string; status: TaskStatus; failure_reason?: string }>(
      ...
      (e) => {
        ...
        // Plain-terminal shells report status under their shell id
        // (`shell:<ownerTaskId>:<index>:<timestamp>`). Remap to the owning sidebar task id so a
        // waiting agent in any split shell lights up that terminal task's badge. Non-shell ids
        // (wrapped agent tasks) keep their current behavior.
        const shellMatch = /^shell:([^:]+):/.exec(task_id);
        const targetId = shellMatch ? shellMatch[1] : task_id;
        updateTaskStatus(targetId, status, undefined, failure_reason);
        if (!isActiveTaskStatus(status)) {
          tm.removeTaskBuffers([targetId]);
        }
        if (status === "done") scheduleForDoneTask(targetId);
      },
    );
  ...
  // Register a local directory as a project and activate it (shared by open-folder / successful clone).
  function registerProjectByPath(path: string) {
  ...
  // Unified sidebar: select a session under a project (can cross projects, auto-switching the active project).
  function handleOpenTaskInProject(projectId: string, taskId: string) {
  ...
  // Unified sidebar: under a project, "directly open a native terminal workspace".
  // The task itself doesn't spin up a PTY — the actual shell is managed by ShellTerminalPanel (which has its own new/multi-terminal bar).
  function handleNewTerminalInProject(projectId: string) {
    const project = projects.find((p) => p.id === projectId);
    if (!project) return;
    ...
    const baseTask: Task = {
      ...
      // perpetual "running" spinner. An agent run inside flips this to running/input_required.
      status: "pending",
      createdAt: Date.now(),
    };
    setTasks((prev) => {
  ...
  // Unified sidebar: create a new AI agent task under a project (open the composer).
  function handleNewTaskInProject(projectId: string) {
    const project = projects.find((p) => p.id === projectId);
    if (!project) return;
    if (activeProject?.id !== projectId) {
      handleProjectClick(project);
    ...
    setExpandedProjectIds((prev) => new Set(prev).add(projectId));
    updateProjectView(projectId, { selectedTaskId: null, isNewTask: true });
  }

  // The active project always stays expanded, so its sessions are easy to see.
  useEffect(() => {
  ...
  function invokeRunTask(task: Task, projectPath: string, images: string[], texts: string[] = []) {
    ...
    // Pure terminal task (default): spin up the shell directly, bypassing the agent pipeline.
    if ((task.kind ?? "shell") === "shell") {
      invoke("open_task_shell", {
        taskId: task.id,
        projectPath,
  ...
  async function handleSubmitTask(
    ...
    // 1) Push the task into state immediately so the view switches to RunningView. Leave the worktree
    //    fields empty for now, to avoid the user lingering on NewTaskView during await create_task_worktree,
    //    which would make it seem unresponsive.
    const baseTask: Task = {
      id: taskId,
    ...
    // 2) The terminal buffer must be set up before the PTY starts, otherwise the first batch of output won't make it into the buffer.
    tm.resetTaskTerminal(taskId);

    // 3) In worktree mode, create the worktree first, then on success fill the fields back into the task before starting the PTY.
    ...
    if (launchMode === "worktree") {
      try {
        ...
        // Roll back the placeholder task we just added
        setTasks((prev) => {
  ...
  async function handleMergeWorktree(taskId: string) {
    ...
    try {
      ...
      // After a successful merge, clean up the worktree and branch to avoid leftover residue
  ...
  async function handleGenerateTaskName(taskId: string) {
    ...
    const project = projects.find((p) => p.id === task.projectId);
    if (!project) return;
    // Pick the field by agent, to avoid choosing wrong when legacy data has both fields set
    const sessionPath =
      task.agent === "codex"
        ? (task.codexSessionPath ?? null)
        : (task.claudeSessionPath ?? null);
    // Snapshot at click time, for concurrency validation after the await completes (guards against the user rerunning/resuming/manually renaming meanwhile)
    ...
    try {
      ...
      // During the await the user may delete the task, rename it, rerun, or resume into a new session →
      // do the validation and write inside the same setTasks updater, to avoid relying on React's synchronous scheduling of the updater.
      setTasks((prev) => {
  ...
  function handleRenameProject(projectId: string, name: string) {
    ...
    if (!trimmed) return;
    setProjects((prev) => {
      const next = prev.map((p) => (p.id === projectId ? { ...p, name: trimmed } : p));
      persistProjects(next, showToast, formatSaveProjectsError);
      return next;
    });
    setActiveProject((prev) => (prev?.id === projectId ? { ...prev, name: trimmed } : prev));
  }
        ...
        {mountedProjects.map((project) => {
          ...
          return (
            <ProjectPage
              ...
              onNewTaskInProject={handleNewTaskInProject}
              ...
              onFilesPanelDefaultOpenChange={setFilesPanelDefaultOpen}
