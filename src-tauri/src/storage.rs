...
pub struct Project {
    ...
    // Default = always shown; old data without this field defaults to false, and false is omitted on serialization to keep the file tidy.
    #[serde(
...
fn project_dir(project_id: &str) -> Result<PathBuf, String> {
    Ok(fastaf_dir()?.join("projects").join(project_id))
}

pub(crate) fn ensure_fastaf_dirs() -> Result<(), String> {
...
/// Atomic write: first write to a unique temp file, then rename it to the target path.
/// The temp file name includes the pid + nanosecond timestamp to avoid temp files overwriting each other during concurrent writes.
