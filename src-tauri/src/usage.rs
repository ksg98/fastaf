...
const CLAUDE_429_BACKOFF_SECS: u64 = 300; // 5 minutes
...
/// The moment the Claude usage API last returned a 429; skip retries within 5 minutes.
        ...
        let handshake = (|| -> Result<(), String> {
            write_json_line(
                ...
                &json!({
                    ...
                    "method": "initialize",
                    "params": {
                        "clientInfo": { "name": "fastaf", "version": env!("CARGO_PKG_VERSION") },
                        "capabilities": {}
                    }
                }),
            )?;
...
async fn read_claude_usage() -> UsageSource<ClaudeUsageData> {
    ...
    // 429 cooldown check: skip outright within 5 minutes of the last rate-limit
    {
