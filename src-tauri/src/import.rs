//! Migration import: discover existing Claude Code / Codex CLI projects and their
//! chat sessions on disk so a user moving to FastAF doesn't have to re-add every
//! repo and lose their history.
//!
...
//!
//! This module only *reads* — it returns what it found. The frontend owns creating
//! the FastAF projects/tasks (it is the single writer for projects.json / tasks.json),
//! which keeps the on-disk state consistent with the app's in-memory state.
...
pub struct DiscoveredSession {
    /// Session id — Claude UUID (file stem) or Codex rollout id. Used as the task id so re-import dedups.
    pub id: String,
    pub path: String,
    pub title: String,
    /// "claude" or "codex".
...
pub struct DiscoveredProject {
    pub path: String,
    pub name: String,
    /// Agents that have sessions for this project ("claude", "codex").
    pub agents: Vec<String>,
    pub session_count: usize,
    pub last_active_ms: i64,
    /// True if a FastAF project already points at this path (frontend default-unchecks these).
...
fn probe_codex_session(path: &Path) -> Option<(String, String, String)> {
    ...
    for line in reader.lines().map_while(Result::ok).take(MAX_PROBE_LINES) {
        ...
        if event_type == "session_meta" {
            if let Some(p) = payload {
                ...
                if id.is_none() {
                    if let Some(i) = p.get("id").and_then(|v| v.as_str()) {
                        id = Some(i.to_string());
                    }
                }
            }
    ...
    // Fall back to the id embedded in the filename (rollout-<ts>-<uuid>.jsonl).
    let id = id.or_else(|| {
        path.file_stem()
            .and_then(|s| s.to_str())
            .and_then(|stem| stem.rsplit('-').next().map(|s| s.to_string()))
    });
...
/// Scan `~/.claude` and `~/.codex` for projects + chat sessions that can be imported.
/// Sessions are grouped by their real working directory; directories that no longer
/// exist on disk are dropped (a stale path can't be opened as a project).
#[tauri::command]
pub fn discover_importable_projects() -> Result<Vec<DiscoveredProject>, String> {
    let mut grouped = Grouped::default();
    scan_claude(&mut grouped);
    scan_codex(&mut grouped);

    // superset.sh repos contribute project paths (no chat sessions of their own).
    let superset: HashMap<String, i64> = scan_superset().into_iter().collect();

    let existing_paths: std::collections::HashSet<String> = crate::storage::load_projects()
        .unwrap_or_default()
        .into_iter()
        .map(|p| p.path)
        .collect();

    // Union of every discovered path: those with agent sessions plus superset repo paths.
