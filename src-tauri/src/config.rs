...
const DEFAULT_CONFIG: &str = r#"# FastAF project configuration
# https://github.com/ksg98/fastaf
...
/// Creates `.fastaf/config.toml` in the project directory if it doesn't already exist.
/// Also ensures `.fastaf/attachments/` exists.
/// Returns the parsed config.
#[tauri::command]
pub fn init_project_config(project_path: String) -> Result<ProjectConfig, String> {
    let fastaf_dir = Path::new(&project_path).join(".fastaf");
    let config_path = fastaf_dir.join("config.toml");
    let attachments_dir = fastaf_dir.join("attachments");
...
/// Reads `.fastaf/config.toml` from the project directory.
...
pub fn read_project_config(project_path: String) -> Result<ProjectConfig, String> {
    let config_path = Path::new(&project_path).join(".fastaf").join("config.toml");
    if !config_path.exists() {
...
/// Writes updated config to `.fastaf/config.toml`, creating the directory if needed.
#[tauri::command]
pub fn write_project_config(project_path: String, config: ProjectConfig) -> Result<(), String> {
    let fastaf_dir = Path::new(&project_path).join(".fastaf");
    fs::create_dir_all(&fastaf_dir).map_err(|e| e.to_string())?;
    let config_path = fastaf_dir.join("config.toml");
