...
#[derive(serde::Serialize)]
pub(crate) struct ProjectContentSearchResult {
    path: String,
    name: String,
    dir: String,
    line: u32,
...
/// Search within the "contents" of project files (VS Code's search-in-files).
/// Reuses `git grep` (no extra dependency, only searches git-tracked files), case-insensitive, literal match.
#[tauri::command]
pub async fn search_project_contents(
    project_path: String,
    query: String,
    limit: Option<usize>,
) -> Result<Vec<ProjectContentSearchResult>, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let root = validate_project_root(&project_path)?;
        ...
        let limit = limit.unwrap_or(120).clamp(1, MAX_FILE_SEARCH_RESULTS);
        ...
        let output = cmd
            .args([
                ...
                "-I", // skip binary files
                "-i", // case-insensitive
                "-F", // literal (fixed string), not regex
                "-e",
                &query,
            ])
            .current_dir(&root)
        ...
        // On "no matches", git grep exits with code 1 and an empty stderr — this is not an error.
        if !output.status.success() {
        ...
        let mut results: Vec<ProjectContentSearchResult> = Vec::new();
        for raw in String::from_utf8_lossy(&output.stdout).lines() {
            if results.len() >= limit {
                break;
            }
            // Of the form "relative/path:123:matched line content"
            let mut parts = raw.splitn(3, ':');
            let rel = match parts.next() {
                Some(r) if relative_git_path_is_safe(r) => r,
                _ => continue,
            };
            let line: u32 = match parts.next().and_then(|n| n.trim().parse().ok()) {
                Some(n) => n,
                None => continue,
            };
            let preview = parts.next().unwrap_or("").trim().chars().take(200).collect();

            let (dir, name) = split_relative_file_path(rel);
            let full_path = root.join(rel);
            results.push(ProjectContentSearchResult {
                path: full_path.to_string_lossy().into_owned(),
                name,
                dir,
                line,
